...
import newsFakerFactory from "./newsFakerFactory";
...
const NewsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = newsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("news").create(elem));
...
export default connect(null, mapDispatch)(NewsFakerDialogComponent);
